...
| WAI-041 | Unit          | Framework utilities           | Retry, outcomes, and pure validators   | Deterministic framework logic passes without API quota     | P0       | Yes, every CI run           |
| WAI-042 | Functional    | `/v1/weather-geo`             | Explicit Nairobi coordinate override   | 200; coordinates; complete geo headers when exposed        | P1       | Yes                         |
...
| WAI-045 | Reliability   | Playwright CI                 | Test fails then passes on retry        | Classified flaky and fails CI; retry count remains visible | P0       | Yes                         |
| WAI-046 | Accuracy      | WeatherAI + Open-Meteo        | Anomaly exceeds tolerance              | Comparison evidence attaches before error is rethrown      | P1       | Yes                         |
